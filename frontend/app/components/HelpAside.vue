...
const props = defineProps<{ active: 'faq' | 'numbers' | 'contact' }>()
...
<template>
  <aside class="help__aside" :aria-label="t('help.aside_label')">
    <button type="button" class="help__aside-tab" :aria-label="t('help.aside_toggle')" aria-expanded="false">
      <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M9 6l6 6-6 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
      </svg>
    </button>
    <p class="help__aside-title">{{ t('seo.help') }}</p>
    <div class="help__nav">
      <NuxtLink
        v-for="item in items"
        :key="item.key"
        class="help__nav-item"
        :class="isActive(item.key) && 'help__nav-item_active'"
        :aria-current="isActive(item.key) ? 'page' : undefined"
        :to="localePath(item.to)"
      >
        <span class="help__nav-icon">
          <svg v-if="item.key === 'faq'" width="24" height="24" viewBox="0 0 24 24" fill="none">
